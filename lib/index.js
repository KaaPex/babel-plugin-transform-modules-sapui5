...
const startWrapper = (0, _core().template)(`
  var _exports = {};
`);const endWrapper = (0, _core().template)(`
  return _exports;
`);
            ...
            Program: {
                exit(path) {
                    ...
                    if (importNames.length) {
                        const sapui5Wrapper = path.pushContainer("body", [buildWrapper({
                            // MODULE_NAME: moduleName,
                            AMD_ARGUMENTS: _core().types.arrayExpression(amdArgs.filter(item => item.value != 'exports')),
                            // COMMONJS_ARGUMENTS: commonjsArgs,
                            // BROWSER_ARGUMENTS: browserArgs,
                            IMPORT_NAMES: importNames.filter(item => item.name != '_exports'),
                            // GLOBAL_TO_ASSIGN: buildBrowserInit(browserGlobals, exactGlobals, this.filename || "unknown", moduleName)
                        })])[0];

                        const sapui5Factory = sapui5Wrapper.get("expression.arguments")[1].get("body");
                        sapui5Factory.pushContainer("directives", directives);
                        sapui5Factory.pushContainer("directives", startWrapper());

                        const newBody = sapui5Factory.pushContainer("body", body)
                        newBody.slice(-1).pop().insertAfter(endWrapper());
                    } else {
                        /* just bush body without changes */
